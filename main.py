...
if "GOOGLE_API_KEY" not in st.secrets:
    st.error("❌ No API key found. Please set GOOGLE_API_KEY in .streamlit/secrets.toml")
    st.stop()
...
model = genai.GenerativeModel(
    ...
    system_instruction=(
        ...
        "Also include suitable loan suggestions."
    ),
)
...
investment_per_year = st.number_input(
    "Investment per Year (in ₹):", min_value=0.0, step=1000.0, format="%.2f"
)
expected_returns = st.number_input(
    "Expected Returns (in %):", min_value=0.0, step=0.1, format="%.2f"
)
...
if st.button("Get Suggestions"):
    if investment_per_year > 0 and expected_returns > 0:
        ...
        try:
            ...
            st.subheader("💡 Suggested Investment Schemes & Loans")
